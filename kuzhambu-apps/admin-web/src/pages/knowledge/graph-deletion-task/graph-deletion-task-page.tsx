...
export const GraphDeletionTaskPage = () => {
    ...
    const [tasks, setTasks] = useState<GraphDeletionTaskRecord[]>(
        graphDeletionTaskMockData.tasks as GraphDeletionTaskRecord[]
    );
    const [detailTask, setDetailTask] = useState<GraphDeletionTaskRecord | null>(null);

    if (!canViewGraph)
        return (
            <KuzhambuPage description="需要知识图谱查看权限。" title="图谱删除任务">
                <KuzhambuAlert title="无权查看图谱删除任务" type="warning" showIcon />
            </KuzhambuPage>
        );
    return (
        <KuzhambuPage description="跟踪删除任务、失败原因和重试结果。" title="图谱删除任务">
                    ...
                    ? tasks.map((task) => (
                          <KuzhambuCard key={task.id} title={task.id}>
                              <KuzhambuSpace>
                                  ...
                                  <KuzhambuButton onClick={() => setDetailTask(task)}>
                                      查看详情
                                  </KuzhambuButton>
                                  {task.status === "FAILED" ? (
                                      <KuzhambuButton
