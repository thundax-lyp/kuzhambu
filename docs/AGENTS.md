...
AI 文档路由入口。目标：少读、读准、避免把参考材料当执行规则。
...
- 治理文档优先于需求、接口和设计文档。
- 工程级规则优先于模块文档。
- `50-prompts/` 只保存人工触发提示词；`60-human/` 只保存人类材料和导入源，默认不读。
...
- 用户明确要求使用 redesign 源材料：先读 `60-human/redesign/INDEX.md`，再按索引读取目标文件。
...
- `30-designs/`: 模块设计、专项设计和临时 RUNBOOK
- `40-readiness/`: PR、发布和运维准备
- `50-prompts/`: 人工触发提示词
- `60-human/`: 人类材料、项目叙事和导入源
