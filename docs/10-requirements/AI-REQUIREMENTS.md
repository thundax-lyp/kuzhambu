...
- 必须支持记录和查看模型检测历史。
- 必须支持为 AI 能力维护业务配置；业务配置创建后 `capability` 作为能力归属键不可编辑，后续只能调整提示词模板、模型、默认参数和启停状态。
- 必须支持模型能力校验，避免将不具备相应能力的模型绑定到业务能力。
- 必须支持翻译、标签、视觉描述、信息融合、问答、条目拆分、图片理解、图片生成、知识图谱、摘要、版本摘要、查询理解、回答生成、实体关系抽取、世系图抽取、提示词优化建议、结构化输出和流式文本等能力的业务配置。
...
- AI Key 不得暴露给前端。
- 模型能力与业务能力不匹配时不得保存映射。
- 已有 AI 业务配置不得切换业务能力；需要变更能力归属时必须新建对应能力配置并停用或删除旧配置。
- 删除模型时，若该模型仍被 AI 业务配置使用，必须阻止删除并提示先修改业务配置。
