...
- 三才图会仍不包含多选批量、复杂视觉资产生产，以及标签/问答前端聚合编辑闭环。
...
| 摘要、标签和问答对内联维护 | 部分完成 | 主表摘要、通用内容 tag/qa CRUD 已实现；手工标签绑定、删除标签同步和 AI 标签确认回写已接通 Knowledge 内容引用闭环 | 问答对与摘要的完整前端内联维护、候选预览和更多确认入口仍未完整对齐 | Classics, Knowledge, AI |
...
| 摘要、标签和问答对展示维护 | 部分完成 | 文档摘要、通用标签/问答对模型与 API 已实现；通用标签写路径已接通 Knowledge 协作与内容引用同步 | 文档页面内联维护调用链与问答对确认/版本化链路未接通 | Classics, Knowledge |
