...
- 复杂业务闭环未接通：权限过滤、AI/Worker/Storage 跨域链路、分享联动和部分确认流程仍需联调。
- 三才图会本轮不包含标签治理、多选批量、删除联动、复杂视觉资产生产、导出、静态展示和分享能力。
...
| 多张配图、缩略预览、放大浏览 | 部分完成 | 图片保存、列表、类型与 storage 对象引用链路已落地 | 文件读取、缩略图生成、放大预览相关接口未完善 | Classics, Storage |
...
| 原图上传、删除和预览 | 部分完成 | 原图保存与列表接口已实现，storage 引用字段已落地 | 删除接口未对外暴露，预览读取未完善 | Classics, Storage |
...
| 需求项 | 状态 | 已完成部分 | 未完成部分 | 责任域 |
| --- | --- | --- | --- | --- |
| 文档查看、创建、编辑、删除 | 部分完成 | 文档分页、详情、时间线、保存、删除接口与服务已实现 | 替换文件和版本协同未完成 | Classics |
| 原始文件关联和替换 | 部分完成 | `storage_object_id` 已设计并入参到位，服务支持替换入口 | 文件存在校验、替换流程和权限收敛未完成 | Classics, Storage |
| 全文阅读和内容安全展示 | 部分完成 | 文本内容/时间字段及阅读接口已实现 | 安全渲染策略与接口约束未完成 | Classics |
...
| 列表标题、标签预览、摘要预览和时间信息 | 已完成 | 详情字段和查询可返回标题、时间等信息，前端可按需结合列表、详情和标签接口展示预览 | 无 | Classics |
...
| 需求项 | 状态 | 已完成部分 | 未完成部分 | 责任域 |
| --- | --- | --- | --- | --- |
| 习俗查看、创建、编辑、删除 | 部分完成 | 条目分页、详情、保存、关键词新增、关键词云、删除接口已落地 | 删除关联清理、可见性批量变更未实现 | Classics |
| 概述、正文、分类、关键词、标签、原文摘录展示 | 已完成 | 核心字段与查询服务到位，关键词与内容可返回，前端可按需结合内容与标签接口展示 | 无 | Classics |
| 列表浏览和关键词搜索 | 已完成 | 列表、关键词/标签/可见性筛选与分页已实现，前端可按需结合详情接口展示联动信息 | 无 | Classics |
| 详情聚合查询 | 已完成 | 详情查询和关键词查询已实现，前端可按需组合详情、关键词与标签信息 | 无 | Classics |
| Markdown 安全渲染 | 部分完成 | `content_format` 与内容字段模型可追踪 | 安全渲染策略与过滤器未实现 | Classics |
| 标签云筛选 | 部分完成 | 通用标签模型、关键词云接口与状态筛选已实现 | 标签云权限过滤与输出限缩未实现 | Classics, Knowledge, System |
