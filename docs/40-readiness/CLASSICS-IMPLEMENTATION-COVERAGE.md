...
- 本次对齐聚焦后台接口闭环，前端页面开发不纳入本次范围。
- 业务闭环未接通：权限过滤、AI/Worker/Storage 跨域链路和部分确认流程仍需联调。
...
| --- | --- | --- | --- | --- |
| 14 个正式门类、卷首辅助内容、卷和条目三级浏览 | 部分完成 | 需求、设计、schema、初始化数据、RUNBOOK 任务已覆盖；分类/卷/条目查询服务与条目查询 API 已实现 | 需求只要求逐级浏览，当前按 `分类 -> 卷 -> 条目` 分拆查询实现，未定义单一聚合入口（`category/volume/entry` 统一视图 API 未提供） | Classics |
...
| 展示原文、译文、标签、配图和状态 | 部分完成 | 条目详情已返回正文、译文、状态等基础字段 | 标签与配图聚合返回未统一，标签与配图聚合 API 未提供 | Classics |
...
| 原图上传、删除和预览 | 部分完成 | 原图保存与列表接口已实现，storage 引用字段已落地 | 删除接口未对外暴露，预览读取与确认态未完成 | Classics, Storage |
...
| 分页、筛选、当前卷搜索和多选 | 部分完成 | 条目分页、筛选、卷过滤、排序查询已实现 | 分页规格治理（50/100/200）、多选结果模型与返回聚合未完整实现 | Classics |
...
| 静态展示包含私有内容确认 | 部分完成 | 设计与确认文本字段已明确；风险状态字段存在 | 私有确认参数、校验与确认开关参数未实现 | Classics |
...
| 列表标题、标签预览、摘要预览和时间信息 | 部分完成 | 详情字段和查询可返回标题、时间等信息 | 列表聚合响应模型未定义 | Classics |
...
| 概述、正文、分类、关键词、标签、原文摘录展示 | 部分完成 | 核心字段与查询服务到位，关键词与内容可返回 | 标签聚合返回模型未定义 | Classics |
| 列表浏览和关键词搜索 | 部分完成 | 列表、关键词/标签/可见性筛选与分页已实现 | 列表与详情查询的联动聚合接口未接通 | Classics |
| 详情聚合查询 | 部分完成 | 详情查询和关键词查询已实现 | 详情聚合返回场景与字段边界未定义 | Classics |
...
| 只读访问页 | 部分完成 | Portal 已提供 targets 查询端点，分享链接详情可查询 | token 校验、确认态参数与访问结果语义未实现 | Classics, System |
...
| 私有内容分享确认文案 | 部分完成 | 需求与文案配置在设计与字段模型中存在 | 参数校验、固定文案语义与确认结果写入未实现 | Classics |
| 目标被删除后占位展示 | 部分完成 | 目标快照和目标状态可持久化 | 删除联动与占位响应策略未实现 | Classics |
...
| 删除前二次确认 | 部分完成 | 设计与交互点已明确 | 接口确认参数与二次确认语义未实现 | Classics |
