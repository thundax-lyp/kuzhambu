...
| --- | --- | --- | --- | --- |
| 14 个正式门类、卷首辅助内容、卷和条目三级浏览 | 已完成 | 需求、设计、schema、初始化数据、RUNBOOK 任务已覆盖；分类/卷/条目查询服务与条目查询 API 已实现 | 无 | Classics |
| 门类和卷稳定排序 | 已完成 | `priority` 规则、schema 约束、service 排序参数与稳定排序 API 已支持 | 无 | Classics |
| 条目查看、创建、编辑、删除 | 部分完成 | 条目查询、详情、保存、删除接口与运行时代码已到位 | 删除后分享目标状态同步、风险态重算未完成 | Classics |
| 编辑标题、门类、卷、原文、译文和标签 | 部分完成 | 条目编辑核心字段（标题/门类/卷/正文等）与保存链路已实现 | 标签协作链路（通用标签联动）和入参校验规则待补齐 | Classics, Knowledge |
| 展示原文、译文、标签、配图和状态 | 已完成 | 条目详情、标签列表、配图列表均已提供独立接口，前端可按需多次请求组合展示 | 无 | Classics |
