...
- 导出和静态展示第一版只记录任务，不同步生成产物；当前已接入导出任务下载门禁与过期控制（过期禁读、前端状态显示），产物生成/回收仍待完善。
...
| 导出记录查看、下载、删除和过期 | 部分完成 | 导出任务列表、下载链接、状态与过期时间透传，后端支持过期任务 404、前端展示“已过期”并禁用下载 | 删除、批量清理策略、权限过滤 API 与产物清理策略未形成闭环 | Classics, System, Storage |
