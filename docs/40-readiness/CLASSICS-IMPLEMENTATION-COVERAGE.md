...
- 当前状态：已完成
- 覆盖范围：三才图会、王圻文档、明代习俗、跨内容分享、通用内容治理、导出、portal 展示、AI 候选、Storage 协作、Discovery 问答入口。
...
- 导出任务继续通过 Worker 渲染并写入 Storage；三才图会展示改为 portal 在线读取公开且已发布内容，不再生成静态展示包。
...
| Portal 展示 | 三才图会公开展示 | 已调整 | portal 在线读取公开且已发布内容，不生成静态展示包 |
