...
- 三类内容页面已接入需求文档要求的“内容上下文内联 AI 精修”任务型入口：Sancai 页面可创建视觉资产相关任务，Wangqi 与 Ming Customs 详情抽屉可分别创建 `summary / tags / qa` 任务；三页都已接入任务轮询，并在 `SUCCEEDED/PARTIAL` 后刷新详情或治理面板。
...
- 导出和静态展示任务治理闭环收口：三类内容页面分别完成导出任务列表/创建闭环、状态可见性、搜索、状态筛选、当前可见多选、单条删除和批量删除；Sancai 的静态展示任务区已完成生成、刷新、搜索、状态筛选、风险筛选、生成时间筛选、预览、下载、当前可见多选、单条删除和批量删除。
...
- 明代习俗统一标签云闭环已补齐：后端 `GET /api/classics/ming-customs/tag-cloud` 基于 `classics_content_tag` 聚合 `tagId/tagNameSnapshot/count`，与列表共用 `category/keyword/visibility` 和权限过滤；Admin Web 标签云 Drawer 点击标签后写入 `tagId/tagNameSnapshot` 筛选，并提供当前标签提示和清除入口。
...
- 结合需求文档、设计文档和代码现状，Classics 当前已实现“内容维护 + 候选确认 + 任务型 AI 入口 + Wangqi 单文档问答入口 + 明代习俗统一标签云 + 三才当前页批量选择 + 三才视觉资产流式候选展示 + 批量视觉资产治理 + 导出/静态展示记录治理 + 批量分享治理 + 删除分享安全闭环 + 单链接多内容分享 + 批量公开/私有分享访问 + 分享恢复治理 + 分享访问统计 + 批量公开/私有状态修改 + 细粒度权限过滤 + 清理协作 + 跨内容批量候选治理”的主干闭环。
...
- 2026-07-08：`cd kuzhambu-servers && mvn -pl biz/classics/kuzhambu-classics-domain,biz/classics/kuzhambu-classics-application,biz/classics/kuzhambu-classics-interface,biz/classics/kuzhambu-classics-infra -am spotless:apply && mvn spotless:check && mvn checkstyle:check && mvn -pl biz/classics/kuzhambu-classics-domain,biz/classics/kuzhambu-classics-application,biz/classics/kuzhambu-classics-interface,biz/classics/kuzhambu-classics-infra -am test` 通过，覆盖 Sancai showcase 记录字段、搜索/状态/风险/生成时间筛选、worker artifact 回源、失败落库、下载门禁和 content disposition。
- 2026-07-08：`cd kuzhambu-apps && pnpm --filter kuzhambu-admin-web run format && pnpm run format:check && pnpm run lint && pnpm run test` 通过，Admin Web 55 个 test files / 231 tests 全绿，覆盖 Sancai 静态展示任务区的搜索、状态筛选、风险筛选、时间筛选、重置、刷新、生成确认、预览、下载和失败提示。
- 2026-07-08：Classics QA 版本治理收口后，已补齐 Admin Web 问答对控件、版本快照展示和服务契约测试；最终验证命令与结果见本轮任务收口记录。
- 2026-07-09：`cd kuzhambu-servers && mvn -pl biz/classics/kuzhambu-classics-domain,biz/classics/kuzhambu-classics-application,biz/classics/kuzhambu-classics-infra,biz/classics/kuzhambu-classics-interface -am spotless:apply && mvn -pl biz/classics/kuzhambu-classics-domain,biz/classics/kuzhambu-classics-application,biz/classics/kuzhambu-classics-infra,biz/classics/kuzhambu-classics-interface -am spotless:check && mvn -pl biz/classics/kuzhambu-classics-domain,biz/classics/kuzhambu-classics-application,biz/classics/kuzhambu-classics-infra,biz/classics/kuzhambu-classics-interface -am checkstyle:check && mvn -pl biz/classics/kuzhambu-classics-domain,biz/classics/kuzhambu-classics-application,biz/classics/kuzhambu-classics-infra,biz/classics/kuzhambu-classics-interface -am test` 通过，覆盖 Ming Customs 统一标签云、权限过滤和相关后端链路。
- 2026-07-09：`cd kuzhambu-apps && pnpm --filter ./admin-web run format && pnpm run format:check && pnpm run lint && pnpm --filter ./admin-web exec vitest run src/pages/classics/common src/pages/classics/wangqi src/pages/classics/ming-customs src/pages/classics/sancai && pnpm --filter ./admin-web run build` 通过，Classics 相关 Admin Web 15 个 test files / 108 tests 全绿。
- 2026-07-09：代码任务完成后的 `cd kuzhambu-apps && pnpm --filter ./admin-web run test` 通过，Admin Web 56 个 test files / 250 tests 全绿。
...
| 分页、筛选、当前卷搜索和多选 | 已完成 | 条目分页、筛选、卷过滤、排序查询已实现；Admin Web 已支持门类/卷筛选、关键词搜索、生命周期筛选、分页、pageSize 切换、批量视觉资产任务、批量分享和当前页多选批量公开/私有，并展示 `当前页已选 N 条`、批量操作成功数、失败数和失败原因；分页或筛选变化后批量选择按当前页数据指纹收敛，不保留旧页选中项；跨页范围动作统一使用筛选结果作用域 | 无 | Classics, Admin Web |
...
| 静态展示内容生成 | 已完成 | 展示任务记录、策略与 worker 产物落库、状态回写和下载能力已完成；静态展示产物使用 `CLASSICS_SANCAI_SHOWCASE + showcase:{classics_sancai_showcase.id}` 绑定 Storage 引用；`GET /api/classics/sancai/assets/showcases/{id}/content` 的 `{id}` 固定为 `classics_sancai_showcase.id`，由 Classics 记录回源读取 `storage_object_id`；非 `COMPLETED` 或无 `storage_object_id` 的记录禁止预览/下载；后端提供 `POST /api/classics/sancai/assets/showcases/delete` 主动删除记录、解绑 owner 并尝试推进无引用 Storage 对象删除；Admin Web 静态展示任务区已支持生成、刷新、搜索、状态筛选、风险筛选、生成时间筛选、重置、预览、下载、当前可见多选、单条删除和批量删除 | 无 | Classics, Worker, Storage, Admin Web |
| 静态展示包含私有内容确认 | 已完成 | `classics_sancai_showcase.visibility_risk_status` 与请求字段 `privateConfirmed` 已接入创建链路；包含私有内容时前端必须勾选确认后才能提交，列表可按风险状态筛选，后端响应保留 `scopeTitle`、`visibilityRiskStatus`、`requestedBy`、`requestedAt`、`completedAt`、`expiresAt`、`filename`、`sizeBytes`、`sha256`、`failureType`、`failureMessage`、`contentUrl` 和 `downloadUrl` | 无 | Classics, Admin Web |
...
| AI 摘要、标签、问答对生成入口和候选确认 | 已完成 | Admin Web 编辑页已接入 AI 候选确认面板，可读取 `PENDING` 候选、编辑 payload、应用或拒绝候选；详情抽屉 `AI 精修任务` 已提供 `创建摘要任务`、`创建标签任务`、`创建问答任务` 和单文档问答入口；任务创建 payload 已按 `summary / tags / qa` 写入 `contentType/contentId/requestId/traceId`，后端已接通候选应用后的正文摘要/标签/问答对写回、版本追加与 AI 候选状态回写 | 无 | Classics, AI |
...
| 需求项 | 状态 | 已完成部分 | 未完成部分 | 责任域 |
| --- | --- | --- | --- | --- |
| 习俗查看、创建、编辑、删除 | 已完成 | 条目分页、详情、保存、关键词新增、关键词云兼容接口、统一标签云和删除接口已落地；Admin Web 已支持列表进入详情、新增、编辑保存、删除和列表刷新闭环 | 无 | Classics, Admin Web |
| 概述、正文、分类、关键词、标签、原文摘录展示 | 已完成 | 核心字段与查询服务到位；Admin Web 已展示列表摘要、分类、关键词、可见性、时间和详情正文，并提供富文本预览 | 无 | Classics, Admin Web |
| 列表浏览和关键词搜索 | 已完成 | 列表、关键词/标签/可见性筛选与分页已实现；Admin Web 已支持标题/分类/可见性筛选、分页和统一标签云点击筛选 | 无 | Classics, Admin Web |
...
| 标签云筛选 | 已完成 | 通用标签模型、关键词云接口与状态筛选已实现；新增统一标签云接口 `GET /api/classics/ming-customs/tag-cloud`，基于 `classics_content_tag` 聚合 `tagId/tagNameSnapshot/count`，并与列表共用 `category/keyword/visibility` 和私有内容权限过滤；Admin Web `标签云` Drawer 点击标签后以 `tagId/tagNameSnapshot` 筛选列表，并展示当前标签筛选和清除入口；旧关键词云保留为兼容能力，不再承载标签云筛选 | 无 | Classics, Knowledge, System |
...
| 摘要、标签和问答对维护 | 已完成 | 通用内容 tag/qa API 已可复用，摘要字段已覆盖；Admin Web 编辑页已接入 `创建摘要任务`、`创建标签任务`、`创建问答任务`、AI 候选确认面板、标签治理面板和问答对治理面板；问答对手工新增/编辑/删除和 AI 候选应用都会进入正式版本治理，明代习俗版本历史面板已展示 `确认标签` 与 `确认问答` 快照，通用标签写路径已接通 Knowledge 协作与内容引用同步 | 无 | Classics, Knowledge, AI |
