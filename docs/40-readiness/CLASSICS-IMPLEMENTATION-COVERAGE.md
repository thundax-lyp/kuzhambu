...
- 复杂业务闭环仍未完全接通：权限过滤、批量操作和部分确认流程仍需联调。
- 三才图会仍不包含多选批量和复杂视觉资产生产。
...
| CSV、JSON、HTML 设定集导出 | 已完成 | 导出任务表、异步接入决策、通用创建能力、worker 渲染产物落库与下载闭环已打通，前端可查看任务状态与下载成功产物 | 删除/清理与批量策略未完成 | Classics, Worker, Storage |
| HTML 视觉资产设定集导出 | 已完成 | 视觉资产字段与导出任务结构已覆盖，worker 渲染产物已写入 Storage，任务状态与下载入口已闭环 | 删除/清理策略未完成 | Classics, Worker, Storage |
| 导出记录查看、下载、删除和过期 | 部分完成 | 导出任务列表、下载链接、状态与过期时间透传，后端支持过期任务 404、前端展示“已过期”并禁用下载，产物已落库存储并可读取 | 删除、批量清理策略、权限过滤 API 与产物清理策略未形成闭环 | Classics, System, Storage |
...
| 需求项 | 状态 | 已完成部分 | 未完成部分 | 责任域 |
| --- | --- | --- | --- | --- |
| 选择三类内容生成分享链接 | 部分完成 | 分享链接创建、状态变更与目标写入服务链路已实现，目标含内容快照字段；Wangqi/MingCustoms/Sancai 页面已提供单内容分享入口，Portal 分享详情可展示固化快照和资源预览 | 跨页面内容选择 UI、多内容冲突校验和创建幂等性未完成 | Classics, Portal Web |
| 分享后台管理（列表、详情、状态、访问记录） | 已完成 | 分享分页列表/详情、状态更新、访问记录分页查询与 Admin Web 页面闭环已完成，支持目标快照与状态来源校验 | 批量分享、权限过滤与部分管理侧告警还未接入 | Classics, Admin Web, System |
