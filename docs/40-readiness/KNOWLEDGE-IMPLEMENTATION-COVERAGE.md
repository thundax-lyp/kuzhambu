...
- 无。当前交付阈值内的 taxonomy 与 Classics 标签协作范围已全部落地到仓库。
...
| 需求项 | 状态 | 已完成部分 | 未完成部分 | 责任域 |
| --- | --- | --- | --- | --- |
| 三才图会实体和关系 AI 提取 | 超出范围 | 需求和边界已沉淀 | 当前交付阈值不包含图谱提取 | Knowledge, AI |
| 异步提取任务和进度展示 | 超出范围 | 需求已沉淀 | 当前交付阈值不包含图谱任务 | Knowledge, AI |
...
| 最近提取版本和提取时间展示 | 超出范围 | 需求已沉淀 | 当前交付阈值不包含图谱版本展示 | Knowledge |
| 世系图专用提取和展示 | 超出范围 | 需求已沉淀 | 当前交付阈值不包含世系图 | Knowledge, AI |

### 运行时验证

| 需求项 | 状态 | 已完成部分 | 未完成部分 | 责任域 |
| --- | --- | --- | --- | --- |
| 当前阈值运行时验证 | 部分完成 | 已完成 taxonomy 后端专项测试、前端 `format`、`lint` 以及 `node scripts/generate-system-data-sql.ts --check` | 缺少前端单测、Playwright 与联调冒烟记录 | Knowledge, Admin Web |
