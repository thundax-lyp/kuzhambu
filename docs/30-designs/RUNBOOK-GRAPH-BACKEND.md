...
2. 按 Mandatory File Plan 的 1-2 完成接口 DTO 和接口单测；每个 URL 至少覆盖权限拒绝、成功映射、错误码映射。
3. 完成 3-4 后运行 deletion service 单测：重复投递、重试、保留贡献、撤回关联、失败后恢复，五项必须通过。
4. 完成 5-8 后覆盖发布并发、冲突未决、撤回不删发布对象、合并/拆分映射完整性和审计快照。
5. 最后新增固定脚本 `scripts/verify-graph-migration.sh`：脚本按 `content_type=SANCAI_ENTRY` 对比迁移前后素材、节点、边、映射数；未定义映射时以非 `0` 退出且禁止自动发布。验证命令和结果只写入 PR 描述，不新增 readiness 文件。
...
1. PR #248 `PRRT_kwDOSkforM6ZJiun`：按 Mandatory File Plan 第 6 项实现 publication conflict decisions。回复：`已在 <SHA> 增加每对象决策并由 preview token 绑定；<command> 通过。`，再 resolve。
2. PR #248 `PRRT_kwDOSkforM6ZJiuq` 与 PR #249 `PRRT_kwDOSkforM6ZJpt5`：按第 6 项实现 preview token 和发布对象版本校验。两条均回复同一 SHA/验证证据，再分别 resolve。
3. PR #248 `PRRT_kwDOSkforM6ZJiuv`：按第 3-4 项实现 deletion change/decision/task；不得继续由 `GraphMaterialEventCommand` 直接删除发布映射。回复包含保留贡献与撤回关联的测试名，再 resolve。
4. PR #248 `PRRT_kwDOSkforM6ZJiuw`：按第 7 项实现 impact token；测试必须在 preview 后新增 incident edge 并断言 delete 返回 `GRAPH_PREVIEW_STALE`，再回复/resolve。
...
3. `Feat(services/knowledge): 重建素材删除变更与任务存储`：仅 deletion change/task 的 domain、infra、DDL、`lockVersion` 乐观锁测试；不含 application service。
4. `Feat(services/knowledge): 实现素材删除预检与决策`：仅 `precheck`、`decide`、change 查询与保留贡献/撤回关联测试。
5. `Feat(services/knowledge): 实现素材删除任务执行与重试`：仅 task 查询、幂等投递、执行、失败恢复、重试和对应测试。
6. `Feat(services/knowledge): 记录图谱治理操作审计`：System audit facade、governance operation/manual source 存储、审计详情查询和测试。
7. `Feat(services/knowledge): 校验图谱发布预览令牌`：publication preview token DDL/domain/infra、冲突决策、版本校验、原子消费和测试。
8. `Feat(services/knowledge): 校验图谱治理影响预览`：impact token DDL/domain/infra、删除/合并/拆分确认校验、级联边变更拒绝测试。
9. `Feat(services/knowledge): 补齐图谱工作台活动读模型`：仅 `recentActivities`、`pendingConflictCount` 的 result/repository/mapper SQL 和测试。
10. `Feat(services/knowledge): 下线旧图谱写入入口`：仅旧写入口下线、只读迁移保护和 `verify-graph-migration.sh`；迁移核对测试必须随该提交完成。
