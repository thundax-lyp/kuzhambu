...
本文档定义 AI 配置与提示词业务域的数据结构和接口设计，按 DDD 风格划分 AI 服务配置、模型能力映射和提示词版本治理的业务边界、聚合和表结构。

AI Config Prompt 是配置治理域，不承载具体业务 AI 候选结果、人工精修、搜索问答编排或知识图谱抽取结果。它只拥有 AI 服务配置、模型条目、能力定义、能力到模型映射、模型检测历史、提示词版本和 AI 调用指标。

## Business Fit Rules

- 业务能力：主备服务、模型列表、能力映射、能力校验、提示词版本、变量校验、回滚、动作可用性检查和调用统计必须能直接对应 AI 配置管理业务。
...
| `model_id` | `modelId` | 是 | 模型 ULID |
| `configured_at` | `configuredAt` | 是 | 配置生效时间 |

约束：
- `capability` 唯一。
- 只能绑定已启用模型。
- 模型能力与业务能力不匹配时不得保存。
- 配置操作人和操作上下文进入 Audit，不进入映射表。
...
| `author_user_id` | `authorUserId` | 是 | 提示词作者 |
| `registered_at` | `registeredAt` | 是 | 登记时间 |
