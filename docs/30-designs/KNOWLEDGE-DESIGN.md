...
- 标签合并用于把源标签的内容引用迁移到目标标签，并保留源标签的合并、别名或废弃状态以支持历史追溯。
- 标签复制用于把源标签下选定或全部内容引用追加到目标标签，不移除源标签原有引用。
- 标签分拆用于把源标签下选定内容引用迁移到新标签或既有目标标签；实现语义等同于先复制选定引用到目标标签，再从源标签移除这些引用。
- 标签废弃禁止后续新增绑定继续使用该标签，但历史引用、统计和审计仍可查看。
- 全局标签改名直接修改 `knowledge_tag` 主数据名称，影响所有引用该 `tagId` 的内容；执行前必须提供影响范围预览，并由 Knowledge 侧记录治理审计。
