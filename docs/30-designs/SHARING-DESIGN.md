...
本文档定义 Sharing 业务域的数据结构与接口设计，按 DDD 风格划分已分享内容和分享链接资产集中管理的数据归属、接口和表结构。
...
- 业务能力：已分享内容集中管理、跨库选内容、单链接多内容、集合创建、公开分享、私有分享、过期、撤销、恢复、只读访问页、访问统计和私有内容公开分享风险提示必须能直接对应 Sharing 业务。
...
- 风险确认人、确认时间、撤销时间和恢复时间进入 Audit；业务表只保留风险确认结果和当前状态。
