...
数据库结构以 `db/schema/ai.sql` 为实现真相源，初始化数据以 `db/data-source/ai-prompts/` 为事实源；`build/seed-sql/ai.sql` 仅为统一导入器生成的临时产物。
...
- 业务能力路由配置。
- 提示词编辑、版本对比和回滚。
- 业务配置负责能力、提示词模板、模型、默认参数和启停状态；调用记录负责展示实际运行结果与失败原因。
- AI 调用统计。
