...
本文档定义运维管理业务域的数据结构和接口设计。本文参考 redesign 中的系统维护、统计分析、备份恢复、日志查看、健康检查和清理任务材料，但不照搬其大对象模型、数据结构或接口形态；本设计必须从管理员查看平台运行状态和执行维护动作的业务流程出发，按 DDD 风格重新划分业务边界、聚合和表结构。
...
- 业务优先于旧设计：运维仪表盘、内容统计、AI 调用统计、访问统计、周报月报、备份恢复、日志筛选、健康检查、过期清理和长任务状态查看必须能直接对应运维业务。
...
- `OperationsDashboard`：运维仪表盘聚合视图，由跨域统计数据实时组装，不长期拥有业务统计明细。
