...
Data Refinement 是人工标注工作台域，不承载摘要编辑、问答对管理、图谱可视化、AI 配置或知识图谱最终展示。它只拥有待精修工作项、实体标注、关系标注、人工确认状态和精修操作日志。
...
- 本模块负责：待精修内容筛选、三才条目精修工作项、实体标注、关系标注、人工确认、删除标记、质量评估抽样标记和精修操作日志。
...
- `DataRefinementOperationLog`：精修操作日志，记录确认、编辑、删除和新增操作。
...
- `RefinementOperationType`：`ENTITY_CONFIRM` / `ENTITY_EDIT` / `ENTITY_DELETE` / `ENTITY_CREATE` / `RELATION_CONFIRM` / `RELATION_EDIT` / `RELATION_DELETE` / `RELATION_CREATE`。
...
| `target_annotation_id` | `targetAnnotationId` | 否 | 目标标注 ULID |
| `before_json` | `beforeJson` | 否 | 操作前快照 |
| `after_json` | `afterJson` | 否 | 操作后快照 |
| `operator_user_id` | `operatorUserId` | 是 | 操作者 |

约束：
- 实体和关系确认、编辑、删除、新增必须写入操作日志。
- 操作日志只记录工作台业务轨迹；全局审计仍由 Audit 业务域记录。
...
- `DataRefinementOperationLogApplicationService`：精修操作轨迹查询。

事务边界：
- 实体或关系保存必须更新对应工作项统计并写入操作日志。
...
- Repository：`DataRefinementWorkItemRepository`、`EntityAnnotationRepository`、`RelationAnnotationRepository`、`DataRefinementOperationLogRepository`。
...
- 本模块拥有：`data_refinement_work_item`、`data_refinement_entity_annotation`、`data_refinement_relation_annotation`、`data_refinement_operation_log`。
