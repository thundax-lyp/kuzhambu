...
本文档定义标签与同义词业务域的数据结构和接口设计，按 DDD 风格划分跨知识库标签治理、标签关联和同义词维护的业务边界、聚合和表结构。

Taxonomy 是治理域，不承载内容正文、AI 标签提取执行、搜索结果排序、问答会话或内容页内编辑。它只拥有统一标签、标签分类、标签别名、内容标签关联和同义词词典；标签治理操作追溯由 Audit 承载。

## Business Fit Rules

- 业务能力：标签审核、分类、合并、废弃、别名、关联内容、使用统计、同义词维护和扩展查询必须能直接对应标签治理业务。
...
- 本模块负责：统一标签、标签分类、标签别名、标签审核、标签合并、标签废弃、内容标签关联、标签使用统计、标签云数据、同义词维护和正反向同义词查询。
...
| `extracted_at` | `extractedAt` | 否 | AI 提取时间 |
...
- 审核人、审核时间、合并操作人和废弃操作人进入 Audit，不进入标签表。
...
事务边界：
- 标签审核通过必须更新标签状态和分类，并由 Audit 记录治理操作。
- 标签审核拒绝必须更新标签状态、清理可用关联，并由 Audit 记录治理操作。
- 标签合并必须在同一事务内迁移关联、迁移别名、创建源标签名称别名、更新源标签状态，并由 Audit 记录治理操作。
- 标签废弃必须更新状态，并由 Audit 记录治理操作。
...
- Repository：`TaxonomyTagRepository`、`TaxonomyCategoryRepository`、`TaxonomyAliasRepository`、`TaxonomyRelationRepository`、`TaxonomySynonymRepository`。
...
- 本模块拥有：`taxonomy_category`、`taxonomy_tag`、`taxonomy_tag_alias`、`taxonomy_content_tag_relation`、`taxonomy_synonym`。
