...
- Apps 子 workspace 目录发生变更时，PR 验证按 workspace 裁剪执行对应 `format:check`、`lint` 和 `test`。`admin-web` 仅变更 `src/pages/<module>/` 时，Vitest 可以裁剪为 `src/app.test.tsx` 和全部受影响 page module；`admin-web` 共享代码、配置或 page module 之外的文件发生变更时必须执行该 workspace 全量测试。`kuzhambu-apps/` 根级文件或 PR workflow 发生变更时必须验证全部 frontend workspace。
