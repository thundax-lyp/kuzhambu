...
- `SERVERS_ANNOTATION_REST_ACTION_VERB_WHITELIST`：REST API 入口类中的公开 HTTP 方法名必须以共享动作白名单开头；`@PostMapping` 方法级路径的最后一个非路径变量片段也必须来自同一白名单。白名单与 admin-web service method ESLint 白名单一一对应；禁用模糊 `save`，新增和更新必须明确使用 `add` / `update`。
