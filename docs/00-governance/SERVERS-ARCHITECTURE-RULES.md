...
- `SERVERS_APPLICATION_COMMAND_PACKAGE`：`*-application` 模块中的 `*Command` 必须位于 `application/{domain}/command/`。
- `SERVERS_APPLICATION_QUERY_PACKAGE`：`*-application` 模块中的 `*Query` 必须位于 `application/{domain}/query/`。
- `SERVERS_APPLICATION_RESULT_PACKAGE`：`*-application` 模块中的 `*Result` 必须位于 `application/{domain}/result/`。
