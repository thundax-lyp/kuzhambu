...
import com.thundax.kuzhambu.storage.infra.object.persistence.mapper.MultipartUploadPartMapper;
import com.thundax.kuzhambu.storage.infra.object.persistence.mapper.MultipartUploadSessionMapper;
...
public class MultipartUploadRepositoryImpl implements MultipartUploadRepository {
    ...
    @Override
    public int deleteByUploadId(MultipartUploadId uploadId) {
        LambdaQueryWrapper<MultipartUploadPartDO> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(MultipartUploadPartDO::getUploadId, MultipartUploadIdCodec.toValue(uploadId));
    ...
    public List<MultipartUploadPart> listMultipartParts(MultipartUploadId uploadId) {
        ...
        for (MultipartUploadPartDO dataObject : dataObjects) {
            parts.add(StoragePersistenceAssembler.toMultipartPartDomain(dataObject));
        }
        return parts;
    }
