...
public class GraphPublication {
    ...
    public static GraphPublication plan(GraphPublicationContext context) {
        ...
        addMissingKeyIssues(context, changes);
        ...
        for (GraphMaterialNode materialNode : context.materialGraph().nodes()) {
            ...
            if (publishedNode == null) {
                ...
                addDeletedObjectIssueIfNeeded(changes, "NODE", keyValue(materialNode.getNodeKey()), publishedNode);
        ...
        for (GraphMaterialEdge materialEdge : context.materialGraph().edges()) {
            ...
            if (publishedEdge == null) {
                ...
                addDeletedObjectIssueIfNeeded(changes, "EDGE", keyValue(materialEdge.getEdgeKey()), publishedEdge);
    ...
    private static void addMissingKeyIssues(GraphPublicationContext context, GraphPublicationChangeSet changes) {
        for (GraphMaterialNode node : context.materialGraph().nodes()) {
            if (node.getNodeKey() == null) {
                changes.addIssue(new GraphPublicationChangeSet.ValidationIssue(
                        "EFFECTIVE_KEY_MISSING", "NODE", null, true, "Graph material node effective key is missing"));
            }
        }
        for (GraphMaterialEdge edge : context.materialGraph().edges()) {
            if (edge.getEdgeKey() == null) {
                changes.addIssue(new GraphPublicationChangeSet.ValidationIssue(
                        "EFFECTIVE_KEY_MISSING", "EDGE", null, true, "Graph material edge effective key is missing"));
            }
        }
    }

    private static String keyValue(BaseStringId key) {
