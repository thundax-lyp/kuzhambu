...
public class GraphMaterialGraph {
    ...
    public void updateNode(GraphMaterialNode node) {
        ...
        refreshEdgesConnectedTo(node.getId());
        material.refreshStatus(nodes.isEmpty());
    }
    ...
    public GraphMaterialChangeSet mergeNodes(
            GraphMaterialNodeId retainedNodeId, List<GraphMaterialNodeId> mergedNodeIds) {
        material.requireEditable();
        ...
        for (GraphMaterialNodeId mergedNodeId :
            ...
            reassignEdges(mergedNode.getId(), retainedNode, changes);
            removeNode(mergedNode, changes);
        }
        deduplicateEdges(changes);
        material.refreshStatus(nodes.isEmpty() && edges.isEmpty());
        return changes;
    }
    ...
    public GraphMaterialChangeSet splitNode(
        ...
        for (GraphMaterialEdgeId edgeId :
            ...
            edge.refreshEdgeKey(splitNode, targetNode, true, Map.of());
    ...
    private void refreshEdgesConnectedTo(GraphMaterialNodeId nodeId) {
        for (GraphMaterialEdge edge : edges) {
            if (edge.connects(nodeId)) {
                edge.refreshEdgeKey(endpoint(edge.getSourceNodeId()), endpoint(edge.getTargetNodeId()), true, Map.of());
            }
        }
    }

    private void reassignEdges(
            GraphMaterialNodeId mergedNodeId, GraphMaterialNode retainedNode, GraphMaterialChangeSet changes) {
        for (GraphMaterialEdge edge : edges) {
            ...
            edge.refreshEdgeKey(source, target, true, Map.of());
            changes.addUpdatedEdge(edge);
        }
    }

    private void removeNode(GraphMaterialNode node, GraphMaterialChangeSet changes) {
        nodes.remove(node);
    ...
    private void deduplicateEdges(GraphMaterialChangeSet changes) {
