...
评价指定时间范围或 PR 范围内的项目健康状况。不要只复述提交记录；要基于需求、设计、PR、commit、代码改动规模和验证证据判断项目是否健康推进。

## 调用方式

本 skill 只用于 slash command 直接调用，不定义额外自然语言触发或适用语义。

调用参数是一个时间范围或 PR 范围，例如：
...
以资深工程负责人和代码健康审查顾问的视角输出判断。必须同时评价：
...
3. 根据 PR/commit 主题识别主要模块，再读取相关需求、接口和设计文档：
...
如果范围是 PR 区间，优先使用 first-parent merge commit 确定 PR 边界，再按 PR 对应 merge commit、分支范围或 commit range 分析。
...
- 指定范围内项目是健康推进、偏高压推进、局部失控，还是需要暂停扩张先收口。
- 代码健康和推进健康是否一致。
- 最关键的判断依据是什么。
