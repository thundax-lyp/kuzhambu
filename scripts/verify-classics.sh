...
if ! grep -q "(1, .*'MARKDOWN'.*'PUBLIC'.*-12622809943000)" "${ROOT_DIR}/db/data/classics.sql"; then
    echo "Missing first imported public markdown wangqi document data" >&2
    exit 1
fi

if ! grep -q "(14, .*'MARKDOWN'.*'PUBLIC'.*-12601814743000)" "${ROOT_DIR}/db/data/classics.sql"; then
