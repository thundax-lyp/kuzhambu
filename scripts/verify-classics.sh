...
if ! grep -q "(1, .*'MARKDOWN'.*'DRAFT'.*'NONE'.*-12622809943000)" "${ROOT_DIR}/db/data/classics.sql"; then
    echo "Missing first imported draft markdown wangqi document data" >&2
    exit 1
fi

if ! grep -q "(14, .*'MARKDOWN'.*'DRAFT'.*'NONE'.*-12601814743000)" "${ROOT_DIR}/db/data/classics.sql"; then
...
actual_wangqi_tags=$(grep -c 'INSERT INTO `classics_content_tag`.*V0FOR1FJX0RPQ1VNRU5U' "${ROOT_DIR}/db/data/classics.sql")
...
actual_wangqi_qa=$(grep -c 'INSERT INTO `classics_content_qa_pair`.*V0FOR1FJX0RPQ1VNRU5U' "${ROOT_DIR}/db/data/classics.sql")
...
actual_ming_entries=$(grep -c 'INSERT INTO `classics_ming_customs_entry`' "${ROOT_DIR}/db/data/classics.sql")
...
actual_ming_tags=$(grep -c 'INSERT INTO `classics_content_tag`.*TUlOR19DVVNUT01T' "${ROOT_DIR}/db/data/classics.sql")
...
actual_ming_qa=$(grep -c 'INSERT INTO `classics_content_qa_pair`.*TUlOR19DVVNUT01T' "${ROOT_DIR}/db/data/classics.sql")
