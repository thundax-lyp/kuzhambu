...
CREATE TABLE IF NOT EXISTS `ai_invocation_log` (
    ...
    `requested_at` BIGINT NOT NULL COMMENT '调用发起时间',
    `completed_at` BIGINT DEFAULT NULL COMMENT '调用完成时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `ai_candidate` (
    ...
    `requested_at` BIGINT NOT NULL COMMENT '候选生成请求时间',
    `applied_at` BIGINT DEFAULT NULL COMMENT '候选被应用到正式业务数据的时间',
    `rejected_at` BIGINT DEFAULT NULL COMMENT '候选被拒绝的时间',
    PRIMARY KEY (`id`),
