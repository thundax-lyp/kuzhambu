...
CREATE TABLE IF NOT EXISTS `ai_image_analysis_cache` (
    ...
    `requested_at` datetime(3) NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_ai_image_analysis_cache_id` (`cache_id`),
    UNIQUE KEY `uk_ai_image_analysis_cache_object_hash` (`object_id`, `content_hash`),
    KEY `idx_ai_image_analysis_cache_hash` (`content_hash`)
