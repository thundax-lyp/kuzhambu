...
INSERT INTO `system_menu` (
    ...
    (45, 8, 18, 19, '业务配置', 'ai:config:view,ai:config:edit', 73, 'VISIBLE', '{"icon":"ai-capability"}', '/ai/business-configs', '_self', 'AI 业务能力、提示词模板和模型绑定配置'),
    ...
    (13, NULL, 25, 44, '知识治理', 'knowledge', 85, 'VISIBLE', '{"icon":"book"}', '/knowledge', '_self', '知识治理根菜单'),
    (14, 13, 26, 39, '知识图谱', 'knowledge:graph:view', 80, 'VISIBLE', '{"icon":"book"}', '/knowledge/graph', '_self', '知识图谱治理分组'),
    (46, 14, 27, 32, '知识抽取', 'knowledge:graph:view', 83, 'VISIBLE', '{"icon":"submissions"}', '/knowledge/graph-extraction', '_self', '知识图谱抽取任务查看'),
    (47, 46, 28, 29, '抽取任务编辑', 'knowledge:graph:edit', 82, 'HIDDEN', '{"icon":"submissions"}', '/knowledge/graph-extraction', '_self', '知识图谱抽取任务创建和重生成权限'),
    (48, 46, 30, 31, '抽取候选应用', 'knowledge:graph:apply', 81, 'HIDDEN', '{"icon":"submissions"}', '/knowledge/graph-extraction', '_self', '知识图谱抽取候选应用权限'),
    (41, 14, 33, 34, '图谱工作台', 'knowledge:refinement:view,knowledge:refinement:edit', 82, 'VISIBLE', '{"icon":"submissions"}', '/knowledge/refinement', '_self', '知识图谱工作台'),
    ...
    (20, 43, 49, 50, '检索统计', 'discovery:search:view', 78, 'VISIBLE', '{"icon":"logs"}', '/discovery/search-statistics', '_self', '检索统计和索引重建'),
    (44, 18, 52, 57, '问答', 'discovery:qa:view,discovery:qa:edit', 75, 'VISIBLE', '{"icon":"open-api"}', '/discovery/qa', '_self', '知识库问答分组'),
    (21, 44, 53, 54, '问答', 'discovery:qa:view,discovery:qa:edit', 75, 'VISIBLE', '{"icon":"open-api"}', '/discovery/qa', '_self', '跨知识库问答'),
    (22, 44, 55, 56, '问答运维', 'discovery:qa:view', 70, 'VISIBLE', '{"icon":"logs"}', '/discovery/qa-admin', '_self', '知识库同步、问答来源和轨迹排查'),
    (23, NULL, 59, 72, '运营运维', 'operations', 70, 'VISIBLE', '{"icon":"dashboard"}', '/operations', '_self', '运营运维根菜单'),
    (24, 23, 60, 61, '运营看板', 'operations:dashboard:view,operations:health:view,operations:health:manage', 70, 'VISIBLE', '{"icon":"dashboard"}', '/operations/dashboard', '_self', '内容、AI、搜索和问答统计'),
    (25, 23, 62, 63, '报表记录', 'operations:report:view,operations:report:edit', 65, 'HIDDEN', '{"icon":"submissions"}', NULL, '_self', '周报月报生成记录'),
    (26, 23, 64, 65, '健康检查', 'operations:health:view', 62, 'VISIBLE', '{"icon":"logs"}', '/operations/health', '_self', '健康检查分页、探针结果和关联告警'),
    (27, 23, 66, 67, '任务台账', 'operations:task:view', 60, 'VISIBLE', '{"icon":"logs"}', '/operations/tasks', '_self', '长任务和批量操作运行状态'),
    (28, 23, 68, 69, '备份恢复', 'operations:backup:view,operations:backup:execute,operations:restore:view,operations:restore:execute', 55, 'VISIBLE', '{"icon":"storage"}', '/operations/backup-restore', '_self', '备份台账、手动备份和恢复入口'),
    (29, 23, 70, 71, '清理维护', 'operations:cleanup:view,operations:cleanup:execute', 50, 'VISIBLE', '{"icon":"menus"}', '/operations/cleanup', '_self', '维护清理任务入口'),
    (30, NULL, 73, 76, '审计中心', 'audit:view', 65, 'VISIBLE', '{"icon":"audit"}', '/audit', '_self', '审计中心根菜单'),
    (31, 30, 74, 75, '审计日志', 'audit:view', 65, 'VISIBLE', '{"icon":"audit-logs"}', '/audit/logs', '_self', '业务审计日志'),
    (32, NULL, 77, 94, '系统管理', 'sys', 60, 'VISIBLE', '{"icon":"system"}', '/system', '_self', '系统管理根菜单'),
    (33, 32, 78, 79, '用户管理', 'sys:user:view,sys:user:edit', 60, 'VISIBLE', '{"icon":"users"}', '/system/users', '_self', '后台用户管理'),
    (34, 32, 80, 81, '部门管理', 'sys:department:view,sys:department:edit', 55, 'VISIBLE', '{"icon":"departments"}', '/system/departments', '_self', '后台部门管理'),
    (35, 32, 82, 83, '角色管理', 'sys:role:view,sys:role:edit', 50, 'VISIBLE', '{"icon":"roles"}', '/system/roles', '_self', '后台角色管理'),
    (36, 32, 84, 85, '菜单管理', 'super', 45, 'VISIBLE', '{"icon":"menus"}', '/system/menus', '_self', '后台菜单管理'),
    (37, 32, 86, 87, '字典管理', 'sys:dict:view,sys:dict:edit', 40, 'VISIBLE', '{"icon":"dictionaries"}', '/system/dictionaries', '_self', '后台字典管理'),
    (38, 32, 88, 91, '存储管理', 'storage', 35, 'VISIBLE', '{"icon":"storage"}', '/storage', '_self', '对象存储管理'),
    (39, 38, 89, 90, '对象管理', 'storage:object:view,storage:object:edit', 35, 'VISIBLE', '{"icon":"storage-objects"}', '/storage/objects', '_self', '对象存储文件管理'),
    (40, 32, 92, 93, '系统日志', 'system:log:view', 30, 'VISIBLE', '{"icon":"logs"}', '/system/logs', '_self', '后台运行日志')
ON DUPLICATE KEY UPDATE
    `parent_id` = VALUES(`parent_id`),
    `lft` = VALUES(`lft`),
    `rgt` = VALUES(`rgt`),
