...
- [ ] `storage-interface request`：补齐 admin 分片上传请求模型
    - 任务类型：执行任务
    - 依据文档：`docs/30-designs/RUNBOOK-STORAGE-FULL-CLOSURE.md`
    - 范围对象：`kuzhambu-servers/biz/storage/kuzhambu-storage-interface/src/main/java/com/thundax/kuzhambu/storage/interfaces/admin/object/controller/request`
    - 处理动作：新增 admin 分片上传四段式 request 模型
    - 验收点：admin 分片上传四段式 request 模型齐全且命名与现有 interface 约定一致
    - 重要度：8/10
...
- [ ] `multipart initiate/uploadPart contracts`：补齐 admin 分片上传前半段契约测试
    ...
    - 处理动作：新增 `initiate / uploadPart` 分片上传 contract test 并锁定核心路由与协议字段
    - 验收点：`initiate / uploadPart` 接口存在契约测试覆盖
    - 重要度：9/10
